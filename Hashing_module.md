...
```
Exit with ctrl+x, y and then enter. Let's break down the contents of this file. It's important to understand what's going on, so you can costumize the code later on for your purposes.
...
for (filenames in my_excel){
	...
	sheetlist<-getSheets(wb)
```
The next loop is nested within with another loop nested inside. This level (call it first level) goes through every excel file in our list, loads in the file into the "wb" dataframe, loads the number of excel sheets into "sheetindex" and then loads the sheets into sheetnames.
```
for (sheetname in sheetindex){
		df<-read_excel(filenames, sheet=sheetname)
		varnum<-grep("[Nname]", df)
```
The second level (which will be continuing after the third loop) goes through every sheet of the workbook (which were stored in sheetindex), loads in the sheet and then the columns containing the expression we are looking for. Let's say we want to hash out every participant's name, then we should look for column names containing "name". It's important to understand that this way the code looks for EVERY column name which contains the matching expression. Make sure that you use an exclusive expression, so only the desired columns will be hashed. The expression is inside the square brackets in the line beginning with "varnum". This is case sensitive, so to make the search the most exlusive, you should enter the upper- and lowercase initials, like in our example.
